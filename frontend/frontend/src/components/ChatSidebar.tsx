...
const ChatSidebar = ({ sidebarOpen, setSidebarOpen, showAllUser, setShowAllUser, users, loggedInUser, chats, selectedUser, setSelectedUser, handleLogout, createChat, onlineUsers }: ChatSidebarProps) => {
  ...
  useEffect(() => {
    if (users) {
      console.log("user is given", users)
    ...
    <aside className={`fixed z-20 sm:static top-0 left-0 h-screen w-80 bg-gray-900 border-r border-gray-700 transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} sm:translate-x-0 transition-transform duration-300 flex flex-col`}>
      ...
      <div className='p-6 border-b border-gray-700'>
        ...
        <div className='flex items-center justify-between'>
          ...
          <button
            className={`p-2.5 rounded-lg transition-colors ${showAllUser ? "bg-red-600 hover:bg-red-700 text-white" : "bg-green-600 hover:bg-green-700 text-white"}`}
            onClick={() => setShowAllUser((prev) => !prev)}>
            {
              showAllUser ? <X className='h-4 w-4' /> : <Plus className='w-4 h-4' />
            }
          ...
          showAllUser ? (
            <div className='space-y-4 h-full'>
              <div className='relative'>
                ...
                <input type="text" placeholder='Search Users...' className='w-full pl-10 pr-4 py-3 bg-gray-800
                   border border-gray-700 text-white placeholder-gray-400 rounded-xl'
                  value={searchQuery}
              ...
              <div className='space-y-2 overflow-y-auto h-full pb-4'>
                {
                  users?.filter((u) =>
                    ...
                    .map((u) => (
                      <button key={u._id} className='w-full text-left p-4 rounded-lg border border-gray-700
                        hover:border-gray-600 hover:bg-gray-800 transition-colors' onClick={() => createChat(u)}>
                        <div className='flex items-center gap-3'>
                          <div className='relative'>
                            <UserCircle className='w-6 h-6 text-gray-300' />
                            {
                              onlineUsers?.includes(u._id) && (
                                <span className='absolute top-0 right-0 translate-x-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-green-500 border-2 border-gray-900'></span>
                          ...
                          <div className='flex-1 min-w-0'>
                            ...
                            <div className={`text-xs mt-0.5 ${onlineUsers.includes(u._id) ? "text-green-500" : "text-gray-400"}`}>
                      ...
                      <div className='flex items-center gap-3'>
                        ...
                        <div className='flex-1 min-w-0'>
                          <div className='flex items-center justify-between mb-1'>
                            ...
                            {unseenCount > 0 &&
                              <div className='bg-red-600 text-white text-xs font-bold rounded-full 
                          ...
                          </div>
                          {
                            ...
                            <div className='flex items-center gap-2'>
                              {isSentByMe ? <CornerUpLeft size={14} className='text-blue-400 text-shrink-0' /> :
      ...
      <div className='p-4 border-t border-gray-700 space-y-2'>
        ...
        <button onClick={handleLogout}
          className='w-full flex items-center gap-3 px-4 py-3 hover:bg-red-600 transition-colors text-red-500 hover:text-white'>
